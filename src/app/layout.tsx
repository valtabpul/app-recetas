...
export default async function RootLayout({
  ...
  return (
    <html lang="es" className="light">
      <body className={`${poppins.className} antialiased text-foreground bg-background flex flex-col min-h-screen`}>
              ...
              {token && userName ? (
                <>
                  ...
                  <form action={logoutAction} className="inline m-0 p-0">
                    <button
                      type="submit"
                      className="text-sm font-medium text-pink-700/80 hover:text-pink-600 transition-colors cursor-pointer bg-transparent border-none outline-none"
                    >
                      Cerrar Sesión
                    </button>
                  </form>
